...
function T1_gamma__transform_g_p_2e(g, t, gamma, o, v)
    t1_exp = zeros(v[end], v[end])
    t1_exp[v, o] = t
    x = I(v[end]) - t1_exp
    y = I(v[end]) + t1_exp'
    ...
    gamma_exp = zeros(v[end], v[end])
    gamma_exp[2:end, 1] = gamma 
    w = I(v[end]) - gamma_exp
    z = I(v[end]) + gamma_exp'
    ...
    return fixed_einsum("pt ,qu ,rm ,sn ,tumn ->pqrs", w, z, x, y, g, optimize="optimal")
end
...
function Omega_ai(F, g, h_p, g_p, s, s2, u, o, v)
    ...
    display(extract_mat(F, "vo", o, v);)
    print("F_ai")
    
    display(extract_mat(g_p, "IIvo", o, v);)
    print("g_IIvo")

    Omega_ai= zeros(v[end] - o[end], o[end])
    B = 2

    Omega_ai[:,:] +=  +1.00000000  * extract_mat(F, "vo", o, v);
    Omega_ai[:,:] +=  -1.00000000  * extract_mat(g_p, "IIvo", o, v);
...
# Correct
function Omega_AI(F, L, g_p ,h_p, s, s2, o, v)
    # Evaluates Omega_0_bj = <bj|..H..|HF> # this need to be a vector
    Omega_AI = zeros(v[end] - 1)
    
    for B in 2: v[end]

        display(extract_mat(s, "AIvo", o, v, B))
        println("s_ai gamma")

        display(extract_mat(s, "VIvo", o, v, B))
        println("s_VI gamma")

        display(extract_mat(h_p, "AI", o, v, B))
        println("h_AI")

        display(extract_mat(F, "ov", o, v, B))
        println("F_ov")

        #display(extract_mat(g_p, "AVov", o, v, B))
        #println("g_AV")       

        display(extract_mat(g_p, "AIoo", o, v, B))
        println("g_AI")

        Omega_AI[B-1] +=  +0.50000000  * extract_mat(h_p, "AI", o, v, B);
        Omega_AI[B-1] = Omega_AI[B-1] .+  -1.0000000  * fixed_einsum("ii->", extract_mat(g_p, "AIoo", o, v, B), optimize="optimal");
        Omega_AI[B-1] = Omega_AI[B-1] .+  +1.0000000  * fixed_einsum("ia,ai->", extract_mat(F, "ov", o, v, B), extract_mat(s, "AIvo", o, v, B), optimize="optimal");
        Omega_AI[B-1] = Omega_AI[B-1] .+  -1.0000000  * fixed_einsum("Bia,Bai->", extract_mat(g_p, "AVov", o, v, B), extract_mat(s, "VIvo", o, v, B), optimize="optimal");
...
function Omega_Ai_ai(F, L, g, h_p, g_p, s, s2, u, o, v)
    ...
    #display(extract_mat(h_p, "II", o, v))
    #println("h_p")
    ...
    for B in 2: v[end]
        ...
        display(extract_mat(g_p, "AIvo", o, v, B))
        println("g_p s1")

        #display(extract_mat(F, "vv", o, v, B))
        #println("F_p")

        #display(extract_mat(s, "AIov", v, o, B))
        #println("s_ai")

        #display(extract_mat(g, "ovov", o, v, B))
        #println("g_ovov")

        #display(extract_mat(s, "AIvo", o, v, B))
        #println("s_Aiai")

        #display(extract_mat(u, "vovo", o, v, B))
        #println("u_vovo")

        #print(-0.50000000  * fixed_einsum("jbkc,aj,bick->ai", extract_mat(g, "ovov", o, v, B), extract_mat(s, "AIvo", o, v, B), extract_mat(u, "vovo", o, v, B), optimize="optimal"))
        #print(-0.50000000  * fixed_einsum("jbkc,bi,ajck->ai", extract_mat(g, "ovov", o, v, B), extract_mat(s, "AIvo", o, v, B), extract_mat(u, "vovo", o, v, B), optimize="optimal"))
        #print("final_value")

        Omega_AI_ai[B-1,:,:] +=  -0.50000000  * extract_mat(g_p, "AIvo",  o, v, B); 
        Omega_AI_ai[B-1,:,:] +=  -0.50000000  * extract_mat(h_p, "II",  o, v, B) * extract_mat(s, "AIvo",  o, v, B); 
...
function build_omega(F, g, g_p, h_p, L, s, t, s2, u, o, v)
    ...
    omega[1 : (v[end]-1)] = Omega_AI(F, L, g_p, h_p, s, s2, o, v)
...
# He         0.10000,       -0.02000,        7.53000", basis="sto-3g")

#mol = pyscf.M(atom= "O 0, 0, 0; H 0, 1, 0; H 0.0 0.0 1", basis="aug-ccPVDZ")

#
# Specify diffierentasis for different ghost atoms
#

mol = pyscf.M(atom="H 0.0, 0.0, 0.0; Li 1.606, 0.0, 0.0", basis= "sto-3g")
...
function sto_3g_g_mo(g, h)

    g[1,1,:,:]  =  [1.66049649,    0.10756785,   -0.13770183   , 0.00000000   , 0.00000000   , 0.05310587,
    0.10756785,    0.36503420,   -0.01316599   , 0.00000000   ,-0.00000000   , 0.04256394,
   -0.13770183,   -0.01316599,    0.39460535   ,-0.00000000   , 0.00000000   , 0.01707854,
    0.00000000,    0.00000000,   -0.00000000   , 0.39635621   , 0.00000000   ,-0.00000000,
    0.00000000,   -0.00000000,    0.00000000   , 0.00000000   , 0.39635621   ,-0.00000000,
    0.05310587,    0.04256394,    0.01707854   ,-0.00000000   ,-0.00000000   , 0.36289223]


    g[1,2,:,:]   =   [0.10756785,    0.01261542,   -0.01073199,    0.00000000,    0.00000000,    0.00882850, 
        0.01261542,   -0.00675370,   -0.00291400,    0.00000000,    0.00000000,    0.00463841,
        -0.01073199,   -0.00291400,    0.01094968,   -0.00000000,   -0.00000000,    0.00385431,
        0.00000000,    0.00000000,   -0.00000000,    0.00420873,    0.00000000,   -0.00000000,
        0.00000000,    0.00000000,   -0.00000000,    0.00000000,    0.00420873,   -0.00000000,
        0.00882850,    0.00463841,    0.00385431,   -0.00000000,   -0.00000000,   -0.00329995]


    g[1,3,:,:] = [-0.13770183,   -0.01073199,    0.02123556,   -0.00000000,   -0.00000000,    -0.00248149,
    -0.01073199,   -0.01487925,    0.00031576,   -0.00000000,   -0.00000000,    -0.00059269,
    0.02123556,    0.00031576,    0.00153815,    0.00000000,    0.00000000,     0.00435667,
    -0.00000000,   -0.00000000,    0.00000000,   -0.00466039,   -0.00000000,    -0.00000000,
    -0.00000000,   -0.00000000,    0.00000000,   -0.00000000,   -0.00466039,    -0.00000000,
    -0.00248149,   -0.00059269,    0.00435667,   -0.00000000,   -0.00000000,    -0.01053362]


    g[1,4,:,:] =  [0.00000000,    0.00000000,   -0.00000000,    0.00975115,    0.00000000,   0.00000000, 
    0.00000000,    0.00000000,   -0.00000000,   -0.00713347,    0.00000000,   0.00000000,
    -0.00000000,   -0.00000000,    0.00000000,    0.01044508,   -0.00000000,  -0.00000000,
    0.00975115,   -0.00713347,    0.01044508,   -0.00000000,   -0.00000000,  -0.00592704,
    0.00385431,   -0.05256801,   -0.00767336,    0.00000000,    0.00000000,     0.03158988,
    0.00435667,   -0.00767336,    0.03683828,   -0.00000000,   -0.00000000,     0.02751048,
    -0.00000000,    0.00000000,   -0.00000000,    0.00206545,    0.00000000,     0.00000000,
    -0.00000000,    0.00000000,   -0.00000000,    0.00000000,    0.00206545,    -0.00000000,
    ...
    g[6,4,:,:] =     [   -0.00000000,   -0.00000000,   -0.00000000,   -0.00592704,    0.00000000,    -0.00000000,
    -0.00000000,    0.00000000,    0.00000000,    0.01888499,   -0.00000000,    -0.00000000,
    -0.00000000,    0.00000000,   -0.00000000,   -0.01392352,    0.00000000,     0.00000000,
    -0.00592704,    0.01888499,   -0.01392352,    0.00000000,    0.00000000,     0.01940538,
    0.00000000,   -0.00000000,    0.00000000,    0.00000000,    0.00000000,    -0.00000000,
    ...
    g[6,5,:,:] =     [   -0.00000000,   -0.00000000,   -0.00000000,    0.00000000,   -0.00592704,    -0.00000000,
    -0.00000000,    0.00000000,    0.00000000,   -0.00000000,    0.01888499,    -0.00000000,
    -0.00000000,    0.00000000,   -0.00000000,    0.00000000,   -0.01392352,    -0.00000000,
    0.00000000,   -0.00000000,    0.00000000,   -0.00000000,    0.00000000,     0.00000000,
    -0.00592704,    0.01888499,   -0.01392352,    0.00000000,    0.00000000,     0.01940538,
    ...
    g[6,6,:,:] =     [    0.36289223,   -0.00329995,   -0.01053362,    0.00000000,    0.00000000,    -0.00308840,
    -0.00329995,    0.45402937,    0.03971056,    0.00000000,   -0.00000000,    -0.13209189,
    -0.01053362,    0.03971056,    0.24200960,    0.00000000,    0.00000000,    -0.04284521,
    0.00000000,    0.00000000,    0.00000000,    0.26826544,    0.00000000,     0.00000000,
    0.00000000,   -0.00000000,    0.00000000,    0.00000000,    0.26826544,     0.00000000,
    -0.00308840,   -0.13209189,   -0.04284521,    0.00000000,    0.00000000,     0.44831637]

h[:,:] =    [-4.72734543,   -0.10064930,    0.15620879,   -0.00000000,   -0.00000000, -0.03744362, 
            -0.10064930,   -1.49225283,   -0.02444392,   -0.00000000,   -0.00000000,  0.04889032,
            0.15620879,   -0.02444392,   -1.12334786,    0.00000000,   -0.00000000,  0.02597546,
            -0.00000000,   -0.00000000,    0.00000000,   -1.13523983,    0.00000000, -0.00000000,
            -0.00000000,   -0.00000000,   -0.00000000,   -0.00000000,   -1.13523983, -0.00000000,
            -0.03744362,    0.04889032,    0.02597546,   -0.00000000,   -0.00000000, -0.95069425]

    return g, h
end

function sto_3g_g_p_mo(g_p, h_p)


    g_p[1,1,:,:] =         [0.387929026294,       0.000164849408,      -0.008337546526,      -0.000000000000,       0.000000000000,      -0.003292222577,
    0.000164849408,       0.319330637063,       0.004924648640,       0.000000000000,      -0.000000000000,      -0.004555693945,
   -0.008337546526,       0.004924648640,       0.262659775113,      -0.000000000000,       0.000000000000,      -0.009323702378,
   -0.000000000000,       0.000000000000,      -0.000000000000,       0.288922543326,      -0.000000000000,       0.000000000000,
    0.000000000000,      -0.000000000000,       0.000000000000,      -0.000000000000,       0.288922543326,       0.000000000000,
    ...
    for _ in 1:1000
        ...
        if abs(F_diff) < 10^(-14) && abs(F_diff_p) < 10^(-14)
...
function display_with_commas(matrix)
...
function construct_hf_p()
    ...
    g   = np.einsum("xyzw, px, qy, rz, sw ->pqrs", g_ao, C', C', C', C', optimize="optimal") 
    ...
    #F_e  = h + 2*fixed_einsum("pqii -> pq", g[:,:,o,o], optimize="optimal") - fixed_einsum("piiq -> pq", g[:,o,o,:], optimize="optimal") #- g_p[1,1,:,:]
    #F_p  = h_p - fixed_einsum("pqii -> pq",  g[:,:,o,o], optimize="optimal")  
    ...
    #display(real.(eigvals(F_e)))
    #display(real.(eigvals(F_p)))

    g, h = sto_3g_g_mo(g,h)
    g_p, h_p  = sto_3g_g_p_mo(g_p, h_p)

    L = 2 * g - permutedims(g, [1, 4, 3, 2])

    display(fixed_einsum("piiq -> pq", g[:,o,o,:], optimize="optimal"))
    display(fixed_einsum("pqii -> pq", g[:,:,o,o], optimize="optimal"))
    display(h)

    display(F_e)
    print("F_e")

    return h_p, g_p, F, F_p, L, g, h
end 

h_p, g_p, F, F_p, L, g, h  = construct_hf_p()
...
amplitudes = zeros(n^2 + n + v[end] - 1 + n*(v[end]-1) +  n^2*(v[end]-1)) 
...
let
    ...
    println(pre_t," pre_t1")

    pre_s = ones(v[end]-1, mol.nao, mol.nao)
...
#    pre_s2[v,o,v,o] = pre_s2[v[end]-1,v,o,v,o] + 4*pre_t2[v,o,v,o]
 ...
 #   end
    ...
    pre_s2 = ones(v[end]-1, mol.nao, mol.nao, mol.nao, mol.nao)
...
# s[3:10,1:2] += reshape(Omega(for the s), nv, no)
        ...
        global s[2:v[end],1, v, o] += reshape(amplitudes[n + v[end]: v[end] - 1 + n + n*(v[end]-1)], (v[end]-1, nv, no)) ./ (pre_s[1:v[end]-1, v, o])
...
#exit()
        global t[v, o] += reshape(amplitudes[v[end]: n + v[end] - 1], (nv, no)) ./ pre_t[v, o]        # global t[a+1:end, :] .= 0.0
        display(t[v, o])
        print("t1 amplitudes")
    
    #t[3, 2] =   -0.092067143602
    #t[6, 2] =    0.001582626342
    #t[6, 1] =   -0.000756481430
    #t[3, 1] =    0.000572024767
    #t[4, 2] =   -0.000000000000
    #t[5, 2] =    0.000000000000
    #t[4, 1] =   -0.000000000000
    #t[5, 1] =    0.000000000000

#    t[3,o] =  [0.00058937,   -0.09317233]
#    t[4,o] =  [0.00000000,    0.00000000]
#    t[5,o] =  [0.00000000,    0.00000000]
#    t[6,o] =  [-0.00074397,    0.00213425]
    ...
    #t[v, o] .= 0
        ...
        #gamma =  [0.006461269239, -0.005369141969, 0.000029880338,  -0.000000000000, 0.000000000000]
        ...
        # T1 - transform integrals
         ...
         global g_pt = T1_gamma__transform_g_p_2e(g_p, t[v,o],gamma,  o, v)

         display(h_pt)
         print("h_t")

        if k % 1 == 0
            println(" --------------Energy at step ", k, " : ", hf.energy_nuc() + CC_Energy(F_t, L_t, g_t, h_pt, g_pt, s, u ,o, v))
        ...
        @show global amplitudes = -build_omega(F_t, g_t, g_pt, h_pt, L_t, s, t2, s2, u, o, v)
