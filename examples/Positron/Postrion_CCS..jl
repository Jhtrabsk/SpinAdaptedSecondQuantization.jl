...
## h_ao and d_ao has been checked, and the sign as been flipped there in both cases. 


using LinearAlgebra

function T1_transform_1e(F, t, o, v)
    t1_exp = zeros(v[end], v[end]) # make a zero by zero matrix
    t1_exp[v, o] = t # insert t in each v,o element
    x = I(v[end]) - t1_exp
    y = I(v[end]) + t1_exp'
...
function Omega_Ai_ai(F, L, g, h_p, g_p, s, s2, u, o, v)
    ...
    for B in 2: v[end]
        ...
        Omega_AI_ai[B-1,:,:] +=  -0.50000000  * extract_mat(g_p, "AIvo", o, v, B); # 1
        Omega_AI_ai[B-1,:,:] +=  -0.50000000  * extract_mat(h_p, "II", o, v) * extract_mat(s, "AIvo", o, v, B); # 0 
...
#mol = pyscf.M(atom= "H 0, 0, 0; Li 1.606 0.0 0.0", basis="aug-ccPVDZ")
mf = scf.RHF(mol)
mf.conv_tol = 1e-10
mf.max_cycle = 1000
coords = [(0.35, 0.6, 0.8)]
...
h_ao = mol.intor("int1e_kin") + mol.intor("int1e_nuc") # Error occurs from here

display(h_ao[:,4])
println("specfic h_ao")
...
L = 2 * g - permutedims(g, [1, 4, 3, 2]) # We should maybe move this more down? 
...
function Density(F_HF, F_p, h_ao, h_p_ao, g_ao, F_prev_p, F_prev, o,)
    ...
    for _ in 1:1000
        ...
        D = 2*np.einsum("pi, iq -> pq", C[:,o], C[:,o]', optimize="optimal")
...
function construcu_hf_p()
    ...
    h = C'*h_ao*C
    g_p = np.einsum("xyzw, px, qy, rz, sw ->pqrs", g_ao, C', C', C', C', optimize="optimal")
    ...
    return h_p, g_p, h, g, F, F_p
end 

h_p, g_p, h, g, F, F_p = construcu_hf_p()
...
amplitudes = zeros(n^2 + n + v[end] - 1 + n*(v[end]-1) +  n^2*(v[end]-1)) # changed this dim
...
#exit()
        ...
        if k % 1 == 0
            println(" --------------Energy at step ", k, " : ", hf.energy_nuc() + CC_Energy(F_t, L_t, g_t, h_p, g_p, s, u ,o, v))
            println(" --------------E_2 at step ", k, " : ", hf.energy_nuc() + E2)
            println("E1 - E0 ", E2 - CC_Energy(F_t, L_t, g_t, h_p, g_p, s, u ,o, v))
            diff = old - hf.energy_nuc() -  CC_Energy(F_t, L_t, g_t, h_p, g_p, s, u ,o, v)
            println(" --------------Difference at step ", k, " : ", diff)
            global old = hf.energy_nuc() + CC_Energy(F_t, L_t, g_t, h_p, g_p, s, u ,o, v)
        ...
        @show global amplitudes = -build_omega(F_t, g_t, g_p, h_p, L_t, s, t2, s2, u, o, v)
