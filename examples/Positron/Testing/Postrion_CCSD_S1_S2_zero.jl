...
function Density(F_HF, F_p, h_ao, h_p_ao, g_ao, F_prev_p, F_prev, o)
    ...
    for _ in 1:1000
        ...
        if abs(F_diff) < 10^(-10) #&& abs(F_diff_p) < 10^(-10)
            ...
            return real.(eigvals(F_HF)), S_half_inv*C #real.(eigvals(F_p)), S_half_inv*C_p, real.(eigvals(F_HF)), S_half_inv*C
        end
...
function construcu_hf_p()
    ...
    e, C  = Density(h_ao, h_ao_p, h_ao, h_ao_p, g_ao, 1, 1, o)
...
#exit()
        ...
        elseif sqrt(sum([abs(e)^2 for (l, e) in enumerate(amplitudes) if l != n^2+2*n+2+(v[end]-o[end])^2*o[end]*(o[end]-1)+(v[end]-o[end])*(o[end]-1)])) < 1e-6
            println("Converged!")
            break
        end
    end
