...
function Jacobian(proj1, proj2, op, n)
    hf_expectation_value(simplify(right_state' * proj1 * bch(op, T, n) * (proj2' * right_state)))
end

function Jacobian_com(proj1, op, n)
    hf_expectation_value(simplify(right_state' * proj1 * commutator(bch(op, T, n), c_one + c_two) * (right_state)))
end


function Jacobian_com_2(proj1, op, n)
    hf_expectation_value(simplify(right_state' * proj1 * bch(commutator(op, c_one + c_two), T, n) * (right_state)))
end
...
o = Jacobian_com_2(deex_braop(10,9), HF, 2)
o = simplify_heavy(o)
o = look_for_tensor_replacements_smart(o, S_AIsymmetry)
#o = look_for_tensor_replacements_smart(o, make_exchange_transformer("t", "u"))
#o = look_for_tensor_replacements_smart(o, make_exchange_transformer("g", "L"))
...
open("file_Jacobian_postrion_ai_T1.txt", "w") do output_file
...
o = Jacobian_com_2(deex_braop(12,11,10,9), HF, 2)
o = simplify_heavy(o)
o = look_for_tensor_replacements_smart(o, S_AIsymmetry)
#o = look_for_tensor_replacements_smart(o, make_exchange_transformer("t", "u"))
#o = look_for_tensor_replacements_smart(o, make_exchange_transformer("g", "L"))
...
open("file_Jacobian_postrion_aibj_T2.py", "w") do output_file
...
# function jacobian_ai()
# o = Jacobian(deex_braop(4,3), deex_braop(4,3), HF, 2)
# o = simplify_heavy(o)
# o = look_for_tensor_replacements_smart(o, S_AIsymmetry)
# o = look_for_tensor_replacements_smart(o, make_exchange_transformer("t", "u"))
# o = look_for_tensor_replacements_smart(o, make_exchange_transformer("g", "L"))
...
# 
# @show Jacobian_ai = jacobian_ai()
# open("file_Jacobian_postrion_ai_all.py", "w") do output_file
# for t in Jacobian_ai.terms
#     println(output_file, print_code_einsum_testing(t, "E", SASQ.IndexTranslation(), ['A','I']))
# end
# end

#function jacobian_ai_com(i, j)
#    o = Jacobian_positron_com(deex_braop(4, 3), HF, 2)
#    o = simplify_heavy(o)
#    o = look_for_tensor_replacements_smart(o, S_AIsymmetry)
#    o = look_for_tensor_replacements_smart(o, make_exchange_transformer("t", "u"))
#    o = look_for_tensor_replacements_smart(o, make_exchange_transformer("g", "L"))
...
## Test
##
# Iterate over i and j and generate the results
# Define the range for i and j
i_range = 4:2:6  # Adjust the range as needed
j_range = 3:2:5  # Adjust the range as needed

# Iterate over i and j and generate the results
for i in i_range
    for j in j_range
        global jacobian_result1  # Explicitly mark it as global
        jacobian_result = jacobian_ai(i, j)  # Add the result to the running sum
        jacobian_result1 += jacobian_ai(i, j)  # Add the result to the running sum
        @show jacobian_result  # Display the current value of jacobian_result
        filename = "file_Jacobian_ai_$(i)_$(j).py"
        open(filename, "w") do output_file
            for t in jacobian_result.terms
                println(output_file, print_code_einsum_testing(t, "E", SASQ.IndexTranslation(), ['A', 'I']))
            end
        end 
    end
...
filename = "file_Jacobian_ai_all.py"
@show jacobian_result1
open(filename, "w") do output_file
    for t in jacobian_result1.terms
        println(output_file, print_code_einsum_testing(t, "E", SASQ.IndexTranslation(), ['A', 'I']))
    end
end

println("Jacobian calculations completed and written to files.")
