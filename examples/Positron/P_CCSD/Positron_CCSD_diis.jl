...
#mport Pkg; Pkg.build("LinearSolve")
...
mol = pyscf.M(atom=" H          0.86681        0.60144        5.00000;
 ...
 He         0.1000         0.1000         7.5000", basis="sto-3g")
 # mol = pyscf.M(atom= "H,          0.86681,        0.60144,        5.00000;
# H         -0.86681,        0.60144,        5.00000;
# O          0.00000        -0.07579         5.00000; 
# He         0.10000,       -0.02000,        7.53000", basis="ccPVDZ")

#mol = pyscf.M(atom= "O 0, 0, 0; H 0, 1, 0; H 0.0 0.0 1", basis="sto-3g")
...
#O          0.00000       -0.07579        0.00000", basis= "cc-PVDZ")
...
function initilize_HF()
    ...
    # h_p_ao = mol.intor("int1e_kin") - mol.intor("int1e_nuc")
...
function Density_p_hf(threshold, max_iterations)
        
        ##
        ##
        ##
    
    h_ao, h_p_ao, g_ao, o, v = initilize_HF()
    ...
    for k in 1:1000
        ...
        h_p = S_half_inv*C_p'*h_ao*C_p*S_half_inv
        println("h_p-matrix")
        display(h_p)
        ...
        if (abs(F_diff) < threshold && abs(F_diff_p) < threshold) || k == max_iterations
...
function Density_HF(F_HF, h_ao, g_ao, F_prev, o)
    ...
    energy(real.(eigvals(F_HF)), S_half_inv*C)

    display(F_HF)
    println("F_HF")

    F_HF = Fock_evaluater(D, S_half_inv, h_ao, g_ao)
    display(F_HF)
    println("F_HF function")
    
    if abs(F_diff) < 10^(-10) #&& abs(F_diff_p) < 10^(-14) 

        ##
        ## Return value
        ## 

            display(real.(eigvals(F_HF))) 
            #return real.(eigvals(F_p)), S_half_inv*C_p, real.(eigvals(F_HF)), S_half_inv*C
            return real.(eigvals(F_HF)), S_half_inv*C
        end
    end
end


function diis_B_matrix(diis_dim)

    B_mat = zeros(diis_dim, diis_dim)
    
    B_mat[end, :] .= -1 
    B_mat[:, end] .= -1
    B_mat[end, end] = 0
    
    b_vector = zeros(diis_dim)
    b_vector[end] = -1

    return B_mat, b_vector
end

function density_evaluater(F_HF, S_half_inv, o)
    ...
    display(D)
    println("D inside fock eval")

    return D, C

end

function Fock_evaluater(D, S_half_inv, h_ao, g_ao)
...
function solve_linear_problem(B_mat, b_vector, k)
...
function error_evaluater(e, w, k)

    e_tot = 0.0

    for i in 1:k
        for j in 1:k
            e_tot += dot(e[i,:], e[j,:]) * w[i] * w[j]
        end      
    end 

    println("this is the error $e_tot")

    return e_tot
end

function diis(F_HF, F_p, h_ao, h_p_ao, g_ao, F_prev_p, F_prev, o)
    ...
    F_tot = zeros(diis_dim, v[end], v[end])
    F = zeros(v[end], v[end]) 
    e = zeros(diis_dim, (v[end]-o[end])*o[end])
    D = zeros(v[end], v[end])
    ...
    B_mat, b_vector = diis_B_matrix(diis_dim+1)
    ...
    println("iteration: $k")
       ...
       #     # Evaluate density and update D_tot
       #     D, C = density_evaluater(F_HF, S_half_inv)
       #     F_tot[updater, :, :] = F_HF

       #     println("F_tot")
       #     display(F_tot)
       #     
       #     # Reshape and store in e
       #     e[updater, :] = reshape(extract_mat(F_HF, "vo", o, v, B), (v[end]-o[end])*o[end])
       # 
       #     # Calculate B matrix elements
       #     for i in 1:2
       #         for j in 1:2
       #             B_mat[i, j] = dot(e[i, :], e[j, :])
       #         end
       #     end
       # 
       #     # Solve the linear problem to find weights
       #     w = solve_linear_problem(B_mat, b_vector, 2)
       # 
       #     # Update D matrix with weights
       #     F_HF = zeros(v[end], v[end])
       #     for i in 1:2
       #         F_HF += F_tot[i, :, :] * w[i]
       #     end
       # 
       ...
       #     e_tot = error_evaluater(e, w, 2)

       # else 

            print("new iteration")

            D, C = density_evaluater(F_HF, S_half_inv, o)
            println("C-coeff")
            display(C)
            F_tot[k, :, :] = F_HF
        
            # Reshape the extracted matrix appropriately
            e[k,:] = reshape(extract_mat(F_HF, "vo", o, v, B), (v[end]-o[end])*o[end])
            display(e[k,:])
            println("Fock ai")
            
            # Calculate the B matrix elements
            for i in 1:k
                for j in 1:k
                    B_mat[i,j] = dot(e[i,:], e[j,:]) # Use dot product for clarity
                end 
            end
            
            # Solve the linear problem to find weights

            w = solve_linear_problem(B_mat, b_vector, k)
            ...
            # Calculate total error
            e_tot = error_evaluater(e, w, k)
        
            # Update B matrix and b vector for the next iteration

            B_mat, b_vector = diis_B_matrix(k+2)
        ...
        E_current = energy(real.(eigvals(F_HF)), S_half_inv*C)

        E = E_current - E_prevous
...
function energy(e, C)
    ...
    #e_p, C_p, e, C  = Density(h_ao, h_p_ao, h_ao, h_p_ao, g_ao, 1, 1, o)
    ...
    println("Energy, pHF:", HF(F, h, h, g, o, v) +  hf.mol.energy_nuc())

    return HF_p(F, h, h, g, o, v) +  hf.mol.energy_nuc() 
end

function construct_hf_p()

    h_ao, h_p_ao, g_ao, o, v = initilize_HF()
    e_p, C_p, e, C  = Density_p_hf(10^(-14), 1000)
    ...
    println("Energy_p$(HF_p(F, h, h_p, g_p, o, v))")
    println("Energy, HF:  $(HF_p(F, h, h_p, g_p, o, v) +  hf.mol.energy_nuc())")


    return h_p, g_p, F, F_p, g, h
end 

function construct_hf_diis()
  
    h_ao, h_p_ao, g_ao, o, v = initilize_HF()

    #e, C  = Density_HF(h_ao, h_p_ao, h_ao, h_p_ao, g_ao, 1, 1, o)
    e, C  = diis(h_ao, h_p_ao, h_ao, h_p_ao, g_ao, 1, 1, o)
    ...
    println("Energy, HF:  $(HF(F, h, h, g, o, v) +  hf.mol.energy_nuc())")
...
function construct_hf()
 
    h_ao, h_p_ao, g_ao, o, v = initilize_HF()

    e, C  = Density_HF(h_ao, g_ao, 1, 1, o)
    #e, C  = diis(h_ao, h_p_ao, h_ao, h_p_ao, g_ao, 1, 1, o)
...
h_p, g_p, F, F_p, g, h  = construct_hf_p()
#F, L, g, h  = construct_hf_diis()
#F, L, g, h  = construct_hf()
...
#           global gamma -= amplitudes[1:v[end]-1] ./ (pre_gamma[1:v[end]-1]*100)
#           global s[2:v[end],1, v, o] -= reshape(amplitudes[n + v[end]: v[end] - 1 + n + n*(v[end]-1)], (v[end]-1, nv, no)) ./ (pre_s[1:v[end]-1, v, o]*100)
#           global t[v, o] -= reshape(amplitudes[v[end]: n + v[end] - 1], (nv, no)) ./ (pre_t[v, o]*100)      # global t[a+1:end, :] .= 0.0
#           global t2[v, o, v, o] -= reshape(amplitudes[v[end] + n + n*(v[end]-1):  v[end] - 1 + n + n*(v[end]-1) + n^2], (nv, no, nv, no)) ./ (pre_t2[v, o, v, o]*100)
...
#        end 
        ...
        elseif sqrt(sum([abs(e)^2 for (l, e) in enumerate(amplitudes) if l != n^2+2*n+2+(v[end]-o[end])^2*o[end]*(o[end]-1)+(v[end]-o[end])*(o[end]-1)])) < 1e-14
            println("Converged!")
            break
        end
    end
