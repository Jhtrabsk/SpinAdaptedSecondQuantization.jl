...
function Density(F_HF, F_p, h_ao, h_p_ao, g_ao, F_prev_p, F_prev, o,)
    ...
    for _ in 1:1000
        ...
        if abs(F_diff) < 10^(-10) && abs(F_diff_p) < 10^(-10)
            ...
            C_pt = zeros(v[end], v[end]) 
            C_t = zeros(v[end], v[end])  

            for i in 1:v[end]

                C_pt[:,i] = S_half_inv*C_p*C_p[:,i] 
                C_t[:,i] = S_half_inv*C*C[:,i]  
            ...
            return real.(eigvals(F_p)), C_pt, real.(eigvals(F_HF)), C_t
        end
...
function construcu_hf_p()
    ...
    F_e  = h + 2*fixed_einsum("pqii -> pq", g, optimize="optimal") - fixed_einsum("piiq -> pq", g, optimize="optimal")
    g_IIrs = fixed_einsum("IIrs -> rs", g_p, optimize="optimal") 
...
#exit()
        ...
        # T1 - transform integrals
         ...
         F_pt = fixed_einsum("iipq->i", g_p, optimize="optimal")
