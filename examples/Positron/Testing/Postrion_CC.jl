...
function Omega_ai(F, g, h_p, g_p, s, s2, u, o, v)
    ...
    Omega_ai[:,:] +=  +1.00000000  * extract_mat(F, "vo", o, v, B); ## this should be zero 
...
# mol = pyscf.M(atom="H 0 0 0; Li 1.6 0 0; He 1.0 2.0 0.0; He 1.2 -1.5 1.0", basis="ccPVDZ")
 mol = pyscf.M(atom= "H 0, 0, 0; Li 1.606 0.0 0.0", basis="6-31G")
#  mol = pyscf.M(atom= "H          0.86681        0.60144        5.00000; H         -0.86681        0.60144        5.00000;   O          0.00000       -0.07579        5.00000;  He         0.10000       -0.02000        7.53000 ", basis="sto3g")
...
#mol = pyscf.M(atom="H 0.0, 0.0, 0.0; Li 1.6, 0.0, 0.0", basis= "sto3g")
...
amplitudes = zeros(n^2 + n + v[end] - 1 + n*(v[end]-1) +  n^2*(v[end]-1)) # chanhed this dim
...
#exit()
        global t[v, o] += reshape(amplitudes[v[end]: n + v[end] - 1], (nv, no)) ./ pre_t[v, o]
       ...
       # global s2 = zeros(mol.nao, mol.nao, mol.nao, mol.nao, mol.nao, mol.nao)
       # global s1 = zeros(mol.nao, mol.nao, mol.nao, mol.nao)
...
#  Fpq = hpq + \sum_i Lpqii
        ...
        elseif sqrt(sum([abs(e)^2 for (l, e) in enumerate(amplitudes) if l != n^2+2*n+2+(v[end]-o[end])^2*o[end]*(o[end]-1)+(v[end]-o[end])*(o[end]-1)])) < 1e-8
            println("Converged!")
            break
        end
    end
