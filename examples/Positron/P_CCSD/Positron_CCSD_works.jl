...
function CC_Energy(F, L, g, h_p, g_p, s, u ,o, v)
    ...
    display(u)
    display(+1.00000000  * fixed_einsum("iajb,aibj->", extract_mat(g, "ovov", o, v, B), extract_mat(u, "vovo", o, v, B), optimize="optimal");)
...
function build_omega(F, g, g_p, h_p, L, s, t, s2, u, o, v)
    ...
    println("AI", omega[1 : (v[end]-1)], "\n")
    println("ai",omega[v[end]: n + v[end] - 1], " ai \n")
    println("Ai_ai",omega[n + v[end]: v[end] - 1 + n + n*(v[end]-1)], "AI,ai")
    println("aibj",omega[v[end] + n + n*(v[end]-1):  v[end] - 1 + n + n*(v[end]-1) + n^2], "aibj")
    println("AI_aibj",omega[v[end] + n + n*(v[end]-1) + n^2: end], "Ai_aibj")
...
## Set up the molecule 
 mol = pyscf.M(atom="H 0 0 0; Li 1.6 0 0", basis="sto3g")
...
#mol = pyscf.M(atom= "O 0, 0, 0; H 0, 1, 0; H 0.0 0.0 1", basis="ccPVDZ")
...
function Density(F_HF, F_p, h_ao, h_p_ao, g_ao, F_prev_p, F_prev, o,)
    ...
    for _ in 1:1000
        ...
        if abs(F_diff) < 10^(-12) && abs(F_diff_p) < 10^(-12)
...
function construcu_hf_p()
    ...
    g_p[2,1,3,1] = 0.0000
    g_p[2,1,4,1] = 0.00449532 
    g_p[2,1,5,1] = 0.01450043 
    g_p[2,1,6,1] = 0 

    g_p[2,1,3,2] = 0 
    g_p[2,1,4,2] = -0.01019432
    g_p[2,1,5,2] = -0.03288355
    g_p[2,1,6,2] = 0 
    
    g_p[3,1,3,1] = 0 
    g_p[3,1,4,1] = 0.01450043
    g_p[3,1,5,1] = -0.00449532
    g_p[3,1,6,1] = 0 

    g_p[3,1,3,1] = 0 
    g_p[3,1,4,1] = -0.03288355
    g_p[3,1,5,1] =  0.01019432
    g_p[3,1,6,1] = 0 
...
h_p, g_p, F, F_p, L, g, h  = construcu_hf_p()
...
#        end 
        ...
        # T1 - transform integrals
         ...
         println("g_p after get_transformation")
         display(g_pt[:,:,:,:])
